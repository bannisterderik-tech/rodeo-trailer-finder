...
/* BEGIN USAGE */
/**
 * <image-slot> Ñ user-fillable image placeholder.
 *
 * Drop this into a deck, mockup, or page wherever you want the user to
 * supply an image. You control the slot's shape and size; the user fills it
 * by dragging an image file onto it (or clicking to browse). The dropped
 * image persists across reloads via a .image-slots.state.json sidecar Ñ
 ...
 *
 * Attributes:
 *   id           Persistence key. REQUIRED for the drop to survive reload Ñ
 *                every slot on the page needs a distinct id.
 *   shape        'rect' | 'rounded' | 'circle' | 'pill'   (default 'rounded')
 *                'circle' applies 50% border-radius; on a non-square slot
 *                that's an ellipse Ñ set equal width and height for a true
 *                circle.
 *   radius       Corner radius in px for 'rounded'.       (default 12)
 *   mask         Any CSS clip-path value. Overrides `shape` Ñ use this for
 ...
 *                shows while src is what's displayed Ñ a user-dropped
 *                image hides it.
 *   credit-href  Optional link for the credit overlay (e.g. the
 *                photographer's profile). http(s) URLs only Ñ anything
 *                else renders the credit as plain text.
 *
 * Size and layout come from ordinary CSS on the element Ñ width/height
 * inline or from a parent grid Ñ so it composes with any layout.
 *
 * Usage:
...
(() => {
  ...
  // 2? a ~600px slot in a 1920-wide deck Ñ retina-sharp without making the
  // sidecar enormous. A 1200px WebP at q=0.85 is ~150-300KB.
  const MAX_DIM = 1200;
  // Raster formats only. SVG is excluded (can carry script; createImageBitmap
  // on SVG blobs is inconsistent). GIF is excluded because the canvas
  // re-encode keeps only the first frame, so an animated GIF would silently
  // go still Ñ better to reject than surprise.
  ...
  const subs = new Set();
  let slots = {};
  // ids explicitly cleared before the sidecar fetch resolved Ñ otherwise
    ...
    loadP = fetch(STATE_FILE)
      ...
      .then((j) => {
        ...
        if (j && typeof j === 'object') {
          ...
          // drop a user image that's only on disk Ñ inherit u from the
  ...
  function setSlot(id, val) {
    ...
    subs.forEach((fn) => fn());
    // A drop is rare + high-value Ñ write immediately so nav-away can't lose
  ...
  class ImageSlot extends HTMLElement {
    ...
    constructor() {
      ...
      root.innerHTML =
        ...
        '</div>' +
        // Outside .frame, like .spill/.ctl Ñ the frame's overflow:hidden +
      ...
      // Shadow-DOM listeners live with the shadow DOM Ñ bound once here so
      ...
      // naturalWidth/Height aren't known until load Ñ re-apply so the cover
      ...
      this._spill.addEventListener('pointerdown', (e) => {
        ...
        if (corner) {
          ...
          // ancestor Ñ deck_stage renders slides scaled-to-fit.
      ...
      // Wheel zoom stays available inside reframe mode as a trackpad nicety Ñ
      // zooms toward the cursor (offset' = cursorá(1-k) + offseták).
    ...
    connectedCallback() {
      // Warn once per page Ñ an id-less slot works for the session but
      ...
      // width%/height% in _applyView encode the frame aspect at call time Ñ
    ...
    // handleEvent Ñ one listener object for all four drag events keeps the
    ...
    handleEvent(e) {
      if (e.type === 'dragenter' || e.type === 'dragover') {
        ...
        // dragenter/leave fire for every descendant crossing Ñ count depth
    ...
    async _ingest(file) {
      ...
      // resumes Ñ bump + capture a generation so stale encodes bail.
      const gen = ++this._gen;
      try {
        const w = this.clientWidth || this.offsetWidth || MAX_DIM;
        const url = await toDataUrl(file, w);
        if (gen !== this._gen) return;
        // Only exit reframe once the new image is in hand Ñ a rejected type
    ...
    // Reframing (pan/resize) is only meaningful for fit=cover Ñ contain/fill
    ...
    // layout box Ñ ResizeObserver fires with a 0?0 rect under display:none,
    ...
    _applyView() {
      ...
      // Width/height and left/top are all frame-% Ñ depends only on the
    ...
    _render() {
      ...
      // tool, so its value isn't guaranteed canvas-originated Ñ only accept
      ...
      // Toggle via style.display Ñ the [hidden] attribute alone loses to
